    ...
    def _register_in_catalog(self):
        try:
            ...
            data = {
                ...
                "mqtt_topics": {
                    "publish": [f"sectors/{os.getenv('SECTOR_ID', 'sector-south')}/pipelines/+/measurements"],
                    "subscribe": [f"sectors/{os.getenv('SECTOR_ID', 'sector-south')}/pipelines/+/commands/valves"]
                },
                "api_endpoint": f"http://localhost:{os.getenv('SERVICE_PORT', 8088)}"
            }
