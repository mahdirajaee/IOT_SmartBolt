...
class RaspberryPiWebService(object):
    ...
    def __init__(self):
        ...
        self.svc_port = int(os.environ["SERVICE_PORT"])
        
        if not self.simulator.initialize():
            logger.error("init failed")
        else:
            self.simulator.start()
            logger.info(f"started on port {self.svc_port}")
...
def _register_south_pi_with_catalog(silent=False):
...
def _south_pi_catalog_heartbeat():
    ...
    while True:
        time.sleep(interval)
        tick += 1
        if _register_south_pi_with_catalog(silent=True):
            _print_banner(
                "CATALOG HEARTBEAT",
                [f"service:  {name}",
                 f"address:  http://{host}:{port}",
...
if __name__ == '__main__':
    ...
    _register_south_pi_with_catalog()
    threading.Thread(target=_south_pi_catalog_heartbeat, daemon=True, name="catalog-heartbeat").start()
